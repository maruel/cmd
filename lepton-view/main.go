...
func initialize(spiName string, spiHz int, i2cName string, i2cHz int) (*lepton.Dev, error) {
	...
	if spiHz != 0 {
		if err := spiBus.LimitSpeed(physic.Frequency(spiHz) * physic.Hertz); err != nil {
			return nil, err
		}
	}
	...
	return lepton.New(spiBus, i2cBus)
}
...
func mainImpl() error {
	...
	dev, err := initialize(*spiName, *spiHz, *i2cName, *i2cHz)
	if err != nil {
		return err
	}
